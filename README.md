...
약 15장 분량을 기준으로 초안 제작에는 약 10분, 정밀 검증에는 약 40~50분이 걸립니다. 이는 **GPT-5.6, 추론 강도 Medium**과 렌더링 도구가 이미 설치된 환경을 기준으로 한 대략적인 시간입니다. 팬아트·이미지 대량 요청은 보통 90~120분을 목표로 진행하지만 이는 강제 상한이 아닙니다. 두 시간을 넘길 가능성이 크면 현재 후보로 마무리할지 탐색을 계속할지 사용자에게 확인합니다.
