...
약 15장 분량을 기준으로 초안 제작에는 약 20분, 정밀 검증에는 약 1시간이 걸립니다. 이 수치는 **Codex GPT-5.6, 추론 강도 Medium**과 렌더링 도구가 이미 설치된 환경에서 측정한 대략적인 기준이며, Claude Code에서는 선택한 모델과 effort에 따라 달라질 수 있습니다. 20~25장 정밀 검증은 40~90분 완료를 목표로 하며, 팬아트·이미지 대량 탐색으로 90분을 넘길 전망이면 현재 후보를 확정할지 탐색을 계속할지 사용자에게 확인합니다.
