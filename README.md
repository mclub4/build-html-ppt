...
약 15장 분량을 기준으로 초안 제작에는 약 10분, 정밀 검증에는 약 40~50분이 걸립니다. 이는 **GPT-5.6, 추론 강도 Medium**과 렌더링 도구가 이미 설치된 환경을 기준으로 한 대략적인 시간입니다. 팬아트·이미지 대량 요청도 기본적으로 전체 120분 안에서 처리하도록 검색 시간과 후보 수를 제한하며, 두 시간을 넘길 수 있는 exhaustive research는 예상 지연을 알리고 사용자가 명시적으로 동의한 경우에만 진행합니다.
