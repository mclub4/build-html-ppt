...
약 15장 분량을 기준으로 초안 제작에는 약 20분, 정밀 검증에는 약 1시간이 걸립니다. 이 수치는 **Codex GPT-5.6, 추론 강도 Medium**과 렌더링 도구가 이미 설치된 환경에서 측정한 대략적인 기준이며, Claude Code에서는 선택한 모델과 effort에 따라 달라질 수 있습니다. 20에서 25장 정밀 검증은 40에서 90분 완료를 목표로 하며, 팬아트·이미지 대량 탐색으로 90분을 넘길 전망이면 현재 후보를 확정할지 탐색을 계속할지 사용자에게 확인합니다.
...
| 디자인 | 1페이지 표지를 최우선 아트 디렉션 대상으로 설계하고, 주제별 테마, 언어·주제에 맞춘 디스플레이/본문 글꼴, 공식 브랜드 자산, WebP 이미지, 열린 레이아웃, 다이어그램 적용 |
...
물리적 산업, 제품, 시설, 장비, 생명과학·의학 연구처럼 실제 대상을 보는 것이 이해에 중요한 주제는 표와 SVG만으로 축약하지 않습니다. 공식 기업·기관 이미지, 제품·시설 사진, 현미경·병리·의료 영상 등 주제에 맞는 시각 자료를 데이터·다이어그램과 함께 배치합니다. 반대로 추상 개념, 기밀 시스템, 이미지가 설명에 기여하지 않는 슬라이드에는 사진을 억지로 넣지 않습니다.

## 이미지 생성 도구

이미지 생성은 선택 사항입니다. Codex App 등에서 ImageGen이 제공되거나 Claude Code에 이미지 생성 플러그인·MCP가 연결된 경우에는 분위기 이미지, 에디토리얼 일러스트, 개념 장면, 배경 제작에 활용합니다. Codex에서도 ImageGen 제공 여부는 실행 환경에 따라 달라질 수 있습니다.

실존 인물·그룹, 출시된 게임과 제품, 실제 장소·행사·인터페이스·연구 결과를 소개할 때는 공식 사진, 실제 스크린샷, 키 아트, 패키지, 기록 이미지 등 출처가 있는 자료를 우선합니다. ImageGen은 분위기 배경, 추상 개념, 가상 시나리오, 장식에만 사용하며 실제 대상을 대신하는 닮은꼴 이미지나 가짜 스크린샷은 만들지 않습니다. 기존 대상을 다루는 표지에도 실제 대상을 알아볼 수 있는 출처 기반 이미지가 최소 하나 들어가야 합니다.
...
생성 도구가 없어도 공식·사용자 제공·라이선스가 확인된 이미지, 웹에서 조사한 이미지와 HTML/CSS/SVG 다이어그램으로 작업을 계속하며 브라우저 사전 검사를 실패 처리하지 않습니다. 사용자가 생성 이미지를 명시적으로 요구했는데 도구가 없다면 임의로 도구·인증 정보·유료 서비스를 설치하거나 구성하지 않고, 기존 이미지로 진행할지 생성 도구를 추가할지 먼저 확인합니다.
...
플러그인 또는 단독 스킬 ZIP만 설치한 환경에는 루트 `package.json`이 없어도 됩니다. 먼저 해당 스킬의 `scripts/check_environment.py`를 실행하고, Playwright/Chromium이 없다는 결과가 나온 뒤 사용자가 설치에 동의한 경우에만 다음 관리형 설치 도구를 실행합니다.
...
- [`Agents365-ai/drawio-skill`](https://github.com/Agents365-ai/drawio-skill)은 편집 가능한 아키텍처, 네트워크 토폴로지, ERD, UML, 시퀀스, BPMN, 스윔레인과 복잡한 흐름도를 만들 때 유용한 선택형 동반 스킬입니다. 로컬 내보내기에는 draw.io 데스크톱 CLI가 필요하고, 대형 자동 배치에는 Graphviz를 선택적으로 사용합니다.
...
- 설치 에이전트는 두 동반 스킬의 설치 여부를 확인한 뒤 누락된 항목만 묻습니다. 둘 다 없다면 `선택 도구인 im-not-ai(한국어 윤문)와 drawio-skill(아키텍처·ERD·UML, draw.io CLI 필요)도 설치할까요?`라고 한 번에 질문합니다. 동의를 받기 전에는 동반 스킬, draw.io, Graphviz 또는 시스템 패키지를 설치하지 않습니다.
...
이 프로젝트의 필수 의존성이 아니며 자동으로 설치되지도 않습니다. 사실, 수치, 고유명사와 출처를 확정한 뒤 최종 문구 윤문 단계에 선택적으로 적용하세요. 설치를 원한다면 해당 저장소의 안내를 별도로 따르세요.

### 기술 다이어그램: drawio-skill

아키텍처, 클라우드·네트워크 토폴로지, ERD, UML, 시퀀스, BPMN, 스윔레인이나 복잡한 데이터 흐름이 필요한 덱에는 [Agents365-ai/drawio-skill](https://github.com/Agents365-ai/drawio-skill)을 함께 쓰는 것을 권장합니다. `.drawio` 원본과 SVG·PNG·PDF 등을 만들 수 있고, 공식 클라우드·Kubernetes·UML 도형과 자체 시각 검토를 지원합니다.

drawio-skill은 선택 의존성이며 로컬 내보내기에 draw.io 데스크톱 CLI가 필요합니다. Graphviz는 대규모 자동 배치가 필요할 때만 선택적으로 설치합니다. 이미 설치되어 있으면 build-html-slides가 적합한 기술 슬라이드에서 별도 요청 없이 사용하고, 간단한 2~3개 요소 도식은 더 빠른 HTML/CSS 또는 인라인 SVG로 유지합니다. 외부 CDN 없이 동작하도록 도형과 아이콘을 포함한 자체 완결형 SVG를 사용하고 편집용 `.drawio` 원본도 함께 보존합니다.
