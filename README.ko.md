...
약 15장 분량을 기준으로 빠른 제작에는 약 10분, 정밀 검증에는 약 1시간이 걸립니다. 이 수치는 **Codex GPT-5.6, 추론 강도 Medium**을 기준으로 한 대략적인 목표이며, 주제 조사와 이미지 탐색량, 선택한 모델, 실행 환경에 따라 달라질 수 있습니다. Quick Draft 결과에는 렌더링이나 검증 근거가 없으며, 필요하면 이후 같은 덱을 Full Validation으로 검증할 수 있습니다. 20에서 25장 정밀 검증은 40에서 90분 완료를 목표로 하며, 팬아트·이미지 대량 탐색으로 90분을 넘길 전망이면 현재 후보를 확정할지 탐색을 계속할지 사용자에게 확인합니다.
...
주제 계열은 색상이나 글꼴을 고정하지 않고 필요한 증거만 규정합니다. 예를 들어 반도체는 실제 장비·시설·공정, 의료 연구는 실제 연구·임상 이미지와 불확실성, 여행은 실제 장소·지도·시간·위치가 우선입니다. AI가 문맥을 읽어 주제 계열과 발표 목적을 먼저 선택한 뒤 구조화된 후보 검색기를 호출하므로, 제목에 `기술`, `AI`, `MSA`가 있다는 이유만으로 어두운 콘솔 테마를 선택하지 않습니다.

정밀 검증의 마지막 squint review는 전체 슬라이드를 작게 축소하고 약한 블러를 적용해 세부 글자를 읽지 않은 상태에서 시각적 위계만 확인합니다. 글자 겹침, 어색한 줄바꿈, 이미지 잘림·왜곡·내용 적합성은 판정할 수 없으며 기존 전체 크기 검토를 대체하지 않습니다. 이미 만든 `normal` 캡처를 재사용하므로 슬라이드를 다시 렌더링하지 않습니다.

## 이미지 생성 도구

이미지 생성은 선택 사항입니다. Codex App 등에서 ImageGen이 제공되거나 Claude Code·Gemini CLI에 호환 이미지 생성 도구가 연결된 경우에는 분위기 이미지, 에디토리얼 일러스트, 개념 장면, 배경 제작에 활용합니다. 플랫폼 이름만 보고 생성기가 있다고 가정하지 않고 현재 세션의 실제 도구를 확인합니다.
...
릴리스의 `.skill` 파일은 Gemini CLI가 직접 설치할 수 있는 ZIP 호환 패키지이며 `SKILL.md`가 아카이브 루트에 있습니다.
...
Quick Draft는 HTML, 발표자 노트, `sources.json`, 자산을 만든 뒤 즉시 종료합니다. 환경 검사, Chromium 렌더링, 자동 검증, AI 시각 검토, 품질 점수 계산을 하지 않으며, 전달할 때 검증하지 않은 초안임을 명시합니다.

Full Validation을 선택한 경우에만 환경 검사를 실행합니다.
...
Full Validation은 개별 스크립트를 기억해 조합하지 않고 단일 진입점으로 실행합니다. 20~25장은 일반적으로 40~90분을 목표로 하며, 이미지 대량 탐색 때문에 이를 넘길 전망이면 현재 후보를 확정할지 탐색을 이어갈지 먼저 확인합니다.
...
~/.gemini/build-html-slides/workspaces/<덱이름>-<경로해시>/ # Gemini CLI
